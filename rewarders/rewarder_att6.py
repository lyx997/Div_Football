...
def calc_reward(rew, prev_obs, obs, skill_reward, opp_num, left_owned_ball):
    ...
    if prev_obs:
        ...
        if skill_div_reward >= 0 or ball_position_r > 1 or not left_owned_ball:
            reward = 5.0*win_reward + 5.0*rew + 0.01*yellow_r + 0.001*ball_position_r + 0.01*not_owned_ball_reward + 0.1*change_ball_owned_reward
        else:
            reward = 0.01*skill_div_reward
