...
def calc_reward(rew, prev_obs, obs, skill_reward, opp_num, left_owned_ball):
    ...
    if prev_obs:
        ...
        if skill_div_reward > 0.2 or ball_position_r > 1 or not left_owned_ball:
            reward = 15.0*win_reward + 10.0*rew + 0.01*yellow_r + 0.003*ball_position_r + 0.01*not_owned_ball_dis_reward + 0.01*owned_ball_dis_reward + 0.1*change_ball_owned_reward 
        else:
            reward = 0.05*skill_div_reward + 0.01*ball_position_r + 0.01*owned_ball_dis_reward + 0.1*change_ball_owned_reward
    return reward