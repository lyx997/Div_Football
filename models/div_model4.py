...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_opp = nn.Linear(arg_dict["feature_dims"]["opp_div"],64)  
        self.fc_ball = nn.Linear(arg_dict["feature_dims"]["ball"],64)
        
        self.fc_cat = nn.Linear(64*6, 256)
    ...
    def forward(self, state_dict):
        ...
        opp_state = state_dict["opp_div"]          
        opp_prime_state = state_dict["opp_div_prime"]
        ...
        opp_prime_embed = self.norm_opp(self.fc_opp(opp_prime_state))
        ball_embed = self.norm_ball(self.fc_ball(ball_state))
        ball_prime_embed = self.norm_ball(self.fc_ball(ball_prime_state))

        if dim_num == 2: 
            cat = torch.cat([player_embed, ball_embed, opp_embed, player_prime_embed, ball_prime_embed, opp_prime_embed], dim=1)
        else:
            cat = torch.cat([player_embed, ball_embed, opp_embed, player_prime_embed, ball_prime_embed, opp_prime_embed], dim=2)
