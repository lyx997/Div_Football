...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.conv2d_smm1 = nn.Conv2d(4, 24, (3,3), stride=1)
        self.conv2d_smm2 = nn.Conv2d(24, 24, (3,3), stride=1)
        self.fc_smm = nn.Linear(150144, 256)
        self.fc_left2 = nn.Linear(36*10,96)
        self.fc_right2 = nn.Linear(36*11,96)
        self.fc_cat = nn.Linear(256+96+96+64+64+48+48,arg_dict["lstm_size"])
        ...
        self.norm_smm = nn.LayerNorm(256)
    ...
    def forward(self, state_dict):
        ...
        [horizon, batch, channel, height, width] = smm.size()
        smm = smm.reshape(horizon*batch_size, channel, height, width)
        smm_feature = F.relu(self.conv2d_smm1(smm))
        smm_feature = F.relu(self.conv2d_smm2(smm_feature))
        smm_feature = smm_feature.reshape(horizon*batch_size, -1).view(horizon, batch_size, -1)
        smm_embed = F.relu(self.norm_smm(self.fc_smm(smm_feature)))
        
        cat = torch.cat([smm_embed, player_embed, ball_embed, left_team_embed, right_team_embed, left_closest_embed, right_closest_embed], 2)
    ...
    def make_batch(self, data):
        ...
        s_player_batch, s_ball_batch, s_left_batch, s_left_closest_batch, s_right_batch, s_right_closest_batch, avail_batch, smm_batch =  [],[],[],[],[],[],[],[]
        s_player_prime_batch, s_ball_prime_batch, s_left_prime_batch, s_left_closest_prime_batch, \
                                                  s_right_prime_batch, s_right_closest_prime_batch, avail_prime_batch, smm_prime_batch =  [],[],[],[],[],[],[],[]
        h1_in_batch, h2_in_batch, h1_out_batch, h2_out_batch = [], [], [], []
        a_batch, m_batch, r_batch, prob_batch, done_batch, need_move_batch = [], [], [], [], [], []
        
        for rollout in data:
            s_player_lst, s_ball_lst, s_left_lst, s_left_closest_lst, s_right_lst, s_right_closest_lst, avail_lst, smm_lst =  [], [], [], [], [], [], [], []
            s_player_prime_lst, s_ball_prime_lst, s_left_prime_lst, s_left_closest_prime_lst, \
                                                  s_right_prime_lst, s_right_closest_prime_lst, avail_prime_lst, smm_prime_lst =  [], [], [], [], [], [], [], []
            ...
            for transition in rollout:
                ...
                smm_prime_lst.append(s_prime["smm"])
            ...
            smm_prime_batch.append(smm_prime_lst)
        ...
        s = {
          "player": torch.tensor(np.array(s_player_batch), dtype=torch.float, device=self.device).permute(1,0,2),
          ...
          "smm": torch.tensor(smm_batch, dtype=torch.float, device=self.device).permute(1,0,2,3,4),
        ...
        s_prime = {
          "player": torch.tensor(np.array(s_player_prime_batch), dtype=torch.float, device=self.device).permute(1,0,2),
          ...
          "smm": torch.tensor(smm_prime_batch, dtype=torch.float, device=self.device).permute(1,0,2,3,4),
