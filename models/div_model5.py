...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_player = nn.Linear(arg_dict["feature_dims"]["player_div"],16)  
        self.fc_ball = nn.Linear(arg_dict["feature_dims"]["ball"],16)
        
        self.fc_cat = nn.Linear(16*2,arg_dict["div_lstm_size"])
        
        self.norm_player = nn.LayerNorm(16)
        self.norm_ball = nn.LayerNorm(16)
        self.norm_cat = nn.LayerNorm(arg_dict["div_lstm_size"])
        
        self.lstm  = nn.LSTM(arg_dict["div_lstm_size"], arg_dict["div_lstm_size"])

        self.fc_pi_z1 = nn.Linear(arg_dict["div_lstm_size"], 32)
        self.fc_pi_z2 = nn.Linear(32, arg_dict["div_num"])
        self.norm_pi_z1 = nn.LayerNorm(32)
