...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_pi_a1 = nn.Linear(arg_dict["lstm_size"]+self.div_num, 164)
        self.fc_pi_a2 = nn.Linear(164, 12)
        self.norm_pi_a1 = nn.LayerNorm(164)
        
        self.fc_pi_m1 = nn.Linear(arg_dict["lstm_size"]+self.div_num, 164)
        self.fc_pi_m2 = nn.Linear(164, 8)
        self.norm_pi_m1 = nn.LayerNorm(164)

        self.fc_v1 = nn.Linear(arg_dict["lstm_size"]+self.div_num, 164)
    ...
    def forward(self, state_dict):
        ...
        s_out = torch.cat([out, skill], 2)
        
        a_out = F.relu(self.norm_pi_a1(self.fc_pi_a1(s_out)))
        a_out = self.fc_pi_a2(a_out)
        logit = a_out + (avail-1)*1e7
        prob = F.softmax(logit, dim=2)
        
        prob_m = F.relu(self.norm_pi_m1(self.fc_pi_m1(s_out)))
        ...
        v = F.relu(self.norm_v1(self.fc_v1(s_out)))
        v = self.fc_v2(v)

        return prob, prob_m, v, h_out, prob_z.squeeze(), out
