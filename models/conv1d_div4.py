...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_player = nn.Linear(arg_dict["feature_dims"]["player"]+self.div_num,64)  
        self.fc_ball = nn.Linear(arg_dict["feature_dims"]["ball"]+self.div_num,64)
    ...
    def forward(self, state_dict):
        ...
        player_embed = self.norm_player(self.fc_player(torch.cat([player_state, skill], 2)))
        ball_embed = self.norm_ball(self.fc_ball(torch.cat([ball_state, skill], 2)))
