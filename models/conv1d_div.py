...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_pi_a3 = nn.Linear(12+self.div_num, 12)
        self.norm_pi_a1 = nn.LayerNorm(164)
        self.norm_pi_a2 = nn.LayerNorm(12)
        
        self.fc_pi_m1 = nn.Linear(arg_dict["lstm_size"], 164)
        self.fc_pi_m2 = nn.Linear(164, 8)
        self.fc_pi_m3 = nn.Linear(8+self.div_num, 8)
        self.norm_pi_m1 = nn.LayerNorm(164)
        self.norm_pi_m2 = nn.LayerNorm(8)
        ...
        self.fc_v3 = nn.Linear(16+self.div_num, 1,  bias=False)
    ...
    def forward(self, state_dict):
        ...
        a_s_out = torch.cat([a_out, skill], 2)
        a_s_out = self.fc_pi_a3(a_s_out)
        logit = a_s_out + (avail-1)*1e7
        ...
        prob_m_s = torch.cat([prob_m, skill], 2)
        prob_m_s = self.fc_pi_m3(prob_m_s)
        prob_m_s = F.softmax(prob_m_s, dim=2)

        prob_z = F.relu(self.norm_pi_z1(self.fc_pi_z1(out)))
        prob_z = self.fc_pi_z2(prob_z)
        prob_z = F.softmax(prob_z, dim=2)

        v = F.relu(self.norm_v1(self.fc_v1(out)))
        v = F.relu(self.norm_v2(self.fc_v2(v)))
        v_s = torch.cat([v, skill], 2)
        v_s = self.fc_v3(v_s)

        return prob, prob_m_s, v_s, h_out, prob_z.squeeze(), out

    def make_batch(self, data):
        # data = [tr1, tr2, ..., tr10] * batch_size
        s_player_batch, s_ball_batch, s_left_batch, s_left_closest_batch, s_right_batch, s_right_closest_batch, avail_batch, skill_batch =  [],[],[],[],[],[],[],[]
        ...
        for rollout in data:
            s_player_lst, s_ball_lst, s_left_lst, s_left_closest_lst, s_right_lst, s_right_closest_lst, avail_lst, skill_lst =  [], [], [], [], [], [], [], []
            ...
            for transition in rollout:
                ...
                h1_div_in, h2_div_in = s["hidden_div"]
            ...
            h1_div_in_batch.append(h1_div_in_lst[0])
            h2_div_in_batch.append(h2_div_in_lst[0])
        ...
        s = {
          ...
          "skill": torch.tensor(skill_batch, dtype=torch.float, device=self.device).squeeze(2).squeeze(2).permute(1,0,2),
          ...
          "hidden_div" : (torch.tensor(h1_div_in_batch, dtype=torch.float, device=self.device).squeeze(1).permute(1,0,2), 
                      torch.tensor(h2_div_in_batch, dtype=torch.float, device=self.device).squeeze(1).permute(1,0,2))
        }
        ...
        s_prime = {
          ...
          "skill": torch.tensor(skill_prime_batch, dtype=torch.float, device=self.device).squeeze(2).squeeze(2).permute(1,0,2),
