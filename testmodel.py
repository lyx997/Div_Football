...
parser.add_argument('--scenario', type=str, default="11_vs_11_kaggle")
...
arg_dict = {
    ...
    "left_model_path" : "kaggle_simulations/agent/model_80725824.tar",
    "right_model_path" : "kaggle_simulations/agent/model_133997184.tar",
    "k_epoch" : 3,
...
def agent(obs):
    ...
    state_dict_tensor = state_to_tensor(state_dict, left_hidden)
    with torch.no_grad():
        a_prob, m_prob, _, left_hidden = left_model(state_dict_tensor)
...
def agent_opp(obs):
    ...
    left_state_dict_tensor = state_to_tensor(left_state_dict, left_hidden)
    right_state_dict = right_fe.encode(right_obs)
    right_state_dict_tensor = state_to_tensor(right_state_dict, right_hidden)
    with torch.no_grad():
...
if args.opp:
    env = footballenv.create_environment(env_name=args.scenario, render=args.render, representation='raw', number_of_left_players_agent_controls=1, number_of_right_players_agent_controls=1)
else:
    env = footballenv.create_environment(env_name=args.scenario, render=args.render, representation='raw', number_of_left_players_agent_controls=1, number_of_right_players_agent_controls=0)
obs = env.reset()
