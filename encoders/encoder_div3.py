...
def state_to_tensor(state_dict, h_in, skill, h_div_in):
    ...
    ball_div_state = torch.from_numpy(state_dict["ball_div"]).float().unsqueeze(0).unsqueeze(0)
...
class FeatureEncoder:
    ...
    def encode(self, obs):
        ...
        ball_div_state = np.concatenate((np.array(obs['ball']), np.array(obs['ball_direction'])*20, [ball_speed*20]))
