...
def state_to_tensor(state_dict, h_in, skill, h_div_in):
    player_state = torch.from_numpy(state_dict["player"]).float().unsqueeze(0).unsqueeze(0)
    player_div_state = torch.from_numpy(state_dict["player_div"]).float().unsqueeze(0).unsqueeze(0)
    opp_div_state = torch.from_numpy(state_dict["opp_div"]).float().unsqueeze(0).unsqueeze(0)
    ...
    state_dict_tensor = {
      "player" : player_state,
      "player_div" : player_div_state,
      "opp_div" : opp_div_state,
      "ball" : ball_state,
...
class FeatureEncoder:
    ...
    def encode(self, obs, opp_num):
        ...
        opp_speed = np.linalg.norm(opp_direction)
        ...
        player_div_state = np.concatenate((obs['left_team'][player_num], player_direction*100, [player_speed*100]))
        
        opp_div_state = np.concatenate((obs['right_team'][opp_num], opp_direction*100, [opp_speed*100]))
